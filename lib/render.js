...
(function (root, factory) {
  ...
  // md-alpha_f_box_outline tagging the Fable quota bar. Without a Nerd Font they
  // would draw as boxes, so render() can swap in text; `gap` separates the
  // cache label from its time, which a glyph needs no space before.
  const NERD_GLYPHS = { cache: '\u{F01BC}', rebuild: '\u{F0450}', fable: '\u{F0BFA}', gap: '' };
  const TEXT_GLYPHS = { cache: 'cch', rebuild: '⟳', fable: 'fbl', gap: ' ' };
  ...
  // The percentage right after its bar, or alone when the bar is squeezed out.
  ...
  // Raised digits and units for the cache's time left; anything else passes
  // through unchanged.
  const SUPERSCRIPT = Object.fromEntries(
    [...'0123456789dhm'].map((c, i) => [c, [...'⁰¹²³⁴⁵⁶⁷⁸⁹ᵈʰᵐ'][i]])
  );
  ...
  function renderCache(cache, glyphs) {
    ...
    if (cache.warm && leftMs > 0) {
      ...
      const time = superscript(fmtDuration(Math.ceil(leftMs / 60000) * 60000));
      ...
      return paint(col, glyphs.cache + glyphs.gap + time + stake);
    }
  ...
  function usageRow(input, usage, width, glyphs) {
    ...
    if (isFable(input)) {
      const fable = renderFable(usage, width, glyphs);
      if (fable) top.push(fable);
    }

    const cache = renderCache(input.prompt_cache, glyphs);
