...
(function (root, factory) {
  ...
  // md-alpha_f_box_outline tagging the Fable quota bar, md-earth tagging the
  // account-wide weekly bar it is a subset of. Without a Nerd Font they
  // would draw as boxes, so render() can swap in text.
  const NERD_GLYPHS = { cache: '\u{F01BC}', rebuild: '\u{F0450}', fable: '\u{F0BFA}', weekly: '\u{F01E7}' };
  ...
  function usageRow(input, usage, width, glyphs) {
    ...
    if (sevenPct != null && sevenPct >= SEVEN_DAY_SHOW_PCT) {
      const seven = renderBar('7d', sevenDay, width, glyphs.weekly);
