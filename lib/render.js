...
(function (root, factory) {
  ...
  function renderCache(cache) {
    ...
    if (cache.warm && leftMs == null) return paint(DIM, CACHE_ICON);
    if (cache.warm && leftMs > 0) {
      ...
      return paint(col, CACHE_ICON + superscript(fmtDuration(Math.ceil(leftMs / 60000) * 60000)));
    }
    ...
    return paint(BLUE, CACHE_ICON + (rebuild ? ` ${fmtTokens(rebuild)} ${REBUILD_ICON}` : ''));
  }
