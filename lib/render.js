...
(function (root, factory) {
  ...
  function renderFable(cached, width) {
    ...
    const shown = renderBar('fbl', { used_percentage: fable.percent, resets_at: fable.resets_at }, width, 'fbl');
  ...
  function usageRow(input, usage, width) {
    ...
    if (model) top.push(paint(pick, model));
    if (effort) top.push(paint(pick, effort));
