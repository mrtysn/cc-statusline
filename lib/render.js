...
(function (root, factory) {
  ...
  function render({ input = {}, cwd, usage = null, topic = null, git = null, home = '', columns = null, icons = true, waiting = 0, alarm = false, peer = null, verdict = null, tag = null }) {
    ...
    function lower({ prefix, path, topic: topicMax, id, name = Infinity }) {
      const middle = [];
      const bottom = [];
      // The tag you gave the session leads, in its own colour, so a tab is
      // found by it at a glance; then the session's name.
      if (tag?.name) middle.push(paint(tagColour(tag.hex), `● ${cut(tag.name, name)}`));
