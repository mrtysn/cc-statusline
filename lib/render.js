...
(function (root, factory) {
  ...
  // system-one's show-mode verdict row (section 9 of the decision-model design
  // doc): the four gate scores from the session's shadow log, in a fixed order.
  // Score to two decimals, no leading zero; a question the log line lacks reads
  // "--". A score whose question is in the log line's `fired` list is yellow —
  // the gate already decided it crossed its own threshold — everything else stays
  // faint, labels included.
  const VERDICT_ORDER = [
    ['irr', 'irreversible'],
    ['for', 'foreign_process'],
    ['net', 'leaves_machine'],
  ...
  function renderVerdict(v) {
    ...
    return VERDICT_ORDER.map(([label, key]) => {
      const raw = v.scores ? v.scores[key] : null;
      const text = typeof raw === 'number' ? fmtScore(raw) : '--';
      return paint(DIM, `${label} `) + paint(fired.has(key) ? YELLOW : DIM, text);
    }).join('  ');
  }
  ...
  function render({ input = {}, cwd, usage = null, topic = null, git = null, home = '', columns = null, icons = true, waiting = 0, alarm = false, peer = null, verdict = null }) {
    ...
    function lower({ prefix, path, topic: topicMax, id, name = Infinity }) {
      const middle = [];
      const bottom = [];
      // The session's name leads the row that says what it is doing.
