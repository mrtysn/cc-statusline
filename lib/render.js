...
(function (root, factory) {
  ...
  function usageRow(input, usage, width) {
    ...
    const model = renderModel(input.model?.display_name || input.model?.id, lit);
    const level = effort && paint(pick, EFFORT_LEVELS[effort] || effort);
    const head = [model, level].filter(Boolean).join(' ');
