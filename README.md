...
| `irr .72  for .05  net .11  ins .02` | On its own row, the last verdict of the system-one Bash hook (`agents-shared`): how likely the session's latest command was irreversible, touched a process the session did not start, sent something off the machine, or installed from the network, as the hook scored them. A score is yellow when the hook's own gate fired on it, faint otherwise; `--` for a score the log line lacks. Drawn only while the app's `verdicts` toggle is on and the session's log exists, read from `${XDG_STATE_HOME:-~/.local/state}/system-one/shadow/<session-id>.jsonl` (its last line, without reading the rest of the file) on every redraw |
...
The `verdicts` toggle at the top right, beside the sound pack, turns the status line's fourth row on and off for every session: it is kept as `verdictRow` in `display.json` beside `sounds.json`, which the app writes with the default (off) on first launch and the status line script reads on every redraw, so a click shows on the next one. The row's source is the system-one Bash hook's per-session log (see the segment table); the app draws nothing from it in the grid. Verdicts, in a session's Directory cell menu, opens a window over that session's log instead: every recent call, newest first, with the time, the first 80 characters of the command, the four scores coloured as on the status line, and the question that fired. It re-reads the file when the window opens and every two seconds after, only when the file has changed, and only its last 512 KB, so a long session's log costs the same as a short one's. One window per session; asking again brings it forward.
