...
The first row covers the model and usage; the second, when there is one, names what the session is about; the third covers the session and where it is running; a fourth, only when turned on in the app (see [Agent Bar Hopping](#agent-bar-hopping)), shows what the system-one decision model made of the session's last Bash command. Narrower rows are spread out with equal gaps around every segment, so all rows start and end in the same columns. The space inside each diamond is at least two columns, matching Claude Code's own indent.
...
| `●● homelab mine` | First on the topic row: the session's dots and tags from Agent Bar Hopping, set by hand or from local-repos-list's tags for its launch directory, each in its colour (a tag without one faint); read from `tags.json` beside `display.json` on every redraw, and absent for an untagged session |
...
| `irr .72  for .05  net .11  ins .02` | On its own row, the last verdict of one system-one hook (`agents-shared`), chosen with the app's `verdicts` toggle: the Bash hook's four gate scores (how likely the session's latest command was irreversible, touched a process the session did not start, sent something off the machine, or installed from the network), the prompt hook's kind and wants-action (`knd cor  act .54`, the kind as the first three letters of the chosen class), or the Stop hook's overlong and needless-table scores (`lng .61  tbl .70`). A score is yellow when the hook's own gate fired on it, faint otherwise; `--` for a score the log line lacks. Drawn only while a hook is chosen and the session's log for it exists, read from `${XDG_STATE_HOME:-~/.local/state}/system-one/shadow/<hook>/<session-id>.jsonl` (its last line, without reading the rest of the file) on every redraw |
...
The `verdicts` toggle, at the right end of the top row beside the tag filter, chooses which hook the status line's fourth row draws, for every session: a click cycles off, bash, prompt, stop, off. It is kept as `verdictRow` plus `verdictHook` in `display.json` beside `sounds.json`, which the app writes with the default (off, no hook) on first launch and the status line script reads on every redraw, so a click shows on the next one; with no hook chosen the row is absent. The row's source is that hook's per-session log (see the segment table); the app draws nothing from it in the grid. Verdicts, in a session's Directory cell menu, opens a window over that session's logs instead, with a hook popup (bash, prompt, stop; it opens on the toggle's hook, or bash): every recent call, newest first, with the time, the first 80 characters of the command or prompt or answer, that hook's scores coloured as on the status line, and the question that fired. It re-reads the file when the window opens and every two seconds after, only when the file has changed, and only its last 512 KB, so a long session's log costs the same as a short one's. One window per session; asking again brings it forward.

Right-click any session row for its menu: copy its path or name, tag it, open its verdicts or its folder in local-repos-list, and resume it (finished) or restart it (live). The Tag menu lists seven preset colour dots, one click and nothing to name, and then named tags, a name with a colour made once with New Tag…; each is an on/off toggle, so a session can carry several of both. Delete Tag takes a tag off every session, and Clear removes what was set on a session by hand. A session also carries the tags [local-repos-list](https://github.com/mrtysn/local-repos-list) gives the folder it was launched in (the most specific one, so a repo nested in a container gets its own), automatic ones such as `mine` included. Marks set by hand are kept by session id, which `claude --resume` keeps, so a restarted session stays tagged. Every mark with a colour is a dot in the narrow untitled column beside the Directory cell, as the heat stripe sits beside the cache, one dot per colour; the tags' names are at the start of the Directory caption, each in its colour or faint without one. The popup beside the `verdicts` toggle narrows the list to sessions with one colour of dot or one tag.

The app talks to local-repos-list only through its versioned `api` commands, so its overrides file stays the one place these tags live: `api snapshot --root <dev root>` (the root is `$DEV_ROOT`, else `~/dev`) prints every folder and every tag with its colour and folders, and each change (`api tag`, `untag`, `color`, `delete-tag`) prints the same snapshot once it is written, which the app applies as it comes. It reads the snapshot at launch and every five minutes, for folders cloned or removed, and watches the directory of the overrides file the snapshot names, since local-repos-list replaces the file by renaming a new one over it: a change made anywhere, in the tool's own page or from a shell, shows within about a second. Each snapshot is copied into `tags.json`, each folder with the colours of its tags, so the status line reads it without running anything. A local-repos-list from before the `api` is read through its `--json` list and changed through its older commands, which the log notes once; Delete Tag and the page link need the `api`. The Tag menu edits the session's folder: every tag in use is a toggle for it (the one git decides stays on), Add Tag to Folder… makes a new one, and Tag Colours sets each tag's colour, a preset, any `#RRGGBB` or none, shared by every folder carrying it. Repo Tags…, at the bottom of the Tag menu, in the Session menu and as `repo tags` beside the filter popup, opens a sheet over the list with every tag: its colour, how many folders carry it, and whether local-repos-list sets it itself. Selecting one shows its colour (the seven presets, any `#RRGGBB`, or Clear) and its folders, each with a Remove button unless git put the tag there, and Add Folder… tags one more. New Tag… asks for a name, a colour and the first folder, the session's own when the sheet was opened from one, since a tag exists only while a folder carries it or it has a colour; Delete Tag, after saying how many folders lose it, takes it off every folder and clears its colour (`container` cannot be deleted). Every change is saved as it is made, and the sheet redraws from each snapshot, including one from an outside change while it is open. Open in local-repos-list, in a session's right-click menu, runs `api gui-url --project <launch directory>`, which starts local-repos-list's server if none is running, and opens the page it names in the browser with that folder selected.

Restart Session, and Restart Outdated Sessions… in the Session menu for every live session on an older Claude Code than the latest release, restarts a session in its own iTerm tab so the tabs keep their order: Claude Code gets a SIGTERM, a clean exit that runs its SessionEnd hooks, and once the shell is back at its prompt the resume command (with the session's model, effort and permission mode) is typed there. Only an idle session with no background agent out is restarted; the bulk action says how many busy ones it left alone. If the tab closed along with Claude Code, the session resumes in a new tab instead.
