...
The first row covers the model and usage; the second covers the session and where it is running. Each row keeps its first segments on the left (model and effort; session ID) and pushes the rest to the right, so both rows end in the same column.
