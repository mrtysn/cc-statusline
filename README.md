...
| `●● homelab` | First on the topic row: the session's preset dots and named tags from Agent Bar Hopping, each in its own colour, set by hand or by a Tag Rule over its launch directory; read from `tags.json` beside `display.json` on every redraw, and absent for an untagged session |
