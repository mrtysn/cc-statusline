...
The first row covers the model and usage; the second, when there is one, names what the session is about; the third covers the session and where it is running; a fourth row is reserved but currently empty (see the segment table) -- the data it would draw from still flows through, for the Agent Bar Hopping app window and any future use, but the status line itself no longer renders it. Narrower rows are spread out with equal gaps around every segment, so all rows start and end in the same columns. The space inside each diamond is at least two columns, matching Claude Code's own indent.
...
The `verdicts` toggle, at the right end of the top row beside the tag filter, gates `readVerdictDisplay`'s read of the session's shadow log (`verdictRow` in `display.json`, on/off only); verdicts are shown in the Agent Bar Hopping app window, not on the terminal row. The toggle separately chooses which hook the Verdicts window opens on: a click cycles off, bash, prompt, stop, off, kept as `verdictHook` beside `verdictRow`. It is kept in `display.json` beside `sounds.json`, which the app writes with the default (off, no hook) on first launch and the status line script reads on every redraw, so a click shows on the next one. Verdicts, in a session's Directory cell menu, opens a window over that session's logs instead, with a hook popup (bash, prompt, stop; it opens on the toggle's hook, or bash): every recent call, newest first, with the time, the first 80 characters of the command or prompt or answer, that hook's scores coloured as on the status line, and the question that fired. It re-reads the file when the window opens and every two seconds after, only when the file has changed, and only its last 512 KB, so a long session's log costs the same as a short one's. One window per session; asking again brings it forward.
