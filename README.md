...
| `● homelab` | First on the topic row, in its own colour: the tag given to the session in Agent Bar Hopping, read from `tags.json` beside `display.json` on every redraw; absent for an untagged session |
...
Right-click any session row for its menu: copy its path or name, tag it, open its verdicts, and resume it (finished) or restart it (live). A tag is a name with a colour, made once with New Tag… and put on any number of sessions; Delete Tag takes it off all of them. Tags are kept by session id in `tags.json`, and `claude --resume` keeps the id, so a restarted session stays tagged. The Tag column shows each session's tag and sorts tagged sessions together, and the popup beside the `verdicts` toggle, shown once a tag exists, narrows the list to one tag.
