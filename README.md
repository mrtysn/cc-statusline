...
| `●● homelab my` | First on the topic row: the session's dots and tags from Agent Bar Hopping, set by hand or from local-repos-list's tags for its launch directory, each in its colour (a tag without one faint); read from `tags.json` beside `display.json` on every redraw, and absent for an untagged session |
