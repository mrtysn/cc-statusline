...
| `● homelab` | First on the topic row: the preset dot and the named tag given to the session in Agent Bar Hopping, each in its own colour, read from `tags.json` beside `display.json` on every redraw; either can be there without the other, and neither for an untagged session |
...
Right-click any session row for its menu: copy its path or name, tag it, open its verdicts, and resume it (finished) or restart it (live). The Tag menu starts with seven preset colour dots, one click and nothing to name; below them are named tags, a name with a colour made once with New Tag… and put on any number of sessions, which Delete Tag takes off all of them. A session can have a dot and a named tag at once. Both are kept by session id in `tags.json`, and `claude --resume` keeps the id, so a restarted session stays tagged. The Directory cell shows them: the dot before the path, the tag's name in its colour at the start of the caption; the popup beside the `verdicts` toggle narrows the list to one dot or one tag.
