...
|---|---|---|---|
| State | `your turn`, `question`, `plan`, `approve?`, a running tool with its time (`Bash 3m`), `working`, or `stopped`, then the permission mode (`· auto`); on a finished row, how it ended (below) | `waiting on 1 agent` while background subagents are still out: your turn and a pending agent are both true at once | The session's transcript, and the event hook for `approve?` |
